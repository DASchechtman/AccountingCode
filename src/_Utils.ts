...
function __Util_ColLetterToIndex(col: string) {
  ...
  if (col.split("").some((x) => !letters.includes(x.toUpperCase()))) {
    return 0;
  }
...
// currently used
function __Util_CreateDateString(date: Date | string, local: boolean = true) {
...
function __Util_GetDateFromDateHeader(date_header: string) {
  const HEADER_TEST = new RegExp(
    `^${PURCHASE_HEADER} \\d{1,2}/\\d{1,2}/\\d{1,2}\\d{1,2}$`,
  );
  if (HEADER_TEST.test(date_header)) {
    return date_header.split(" ")[2];
  }
  return "";
}

// currently used
function __Util_GroupAndHighlightOneWeekLoans(
  should_shade_red: boolean = true,
) {
  ...
  SHEET.ForEachRow((row, i, range) => {
    if (
      last_date_header === -1 &&
      String(row[PURCHASE_LOCATION_INDEX]).startsWith(PURCHASE_HEADER)
    ) {
      date = __Util_GetDateFromDateHeader(String(row[PURCHASE_LOCATION_INDEX]));
      last_date_header = i;
    } else if (
      String(row[PURCHASE_LOCATION_INDEX]).startsWith(PURCHASE_HEADER) ||
      i === SHEET.NumberOfRows() - 1
    ) {
    ...
    if (
      __Util_CompareDates(CUR_DATE, date) &&
      !LIGHT_RED_SHADES.includes(BG_COLOR)
    ) {
      range.setBackground(
        LIGHT_RED_SHADES[color_index % LIGHT_RED_SHADES.length],
      );
    }
  }, 1);
}
...
// currently used
function __Util_CreateHeadersForOneWeekLoans(
  date_header: string,
  tab_name: string,
) {
  ...
  const GetTipValue = (date: string) => {
    const TIP = PropertiesService.getDocumentProperties().getProperty(
      `TIPS - ${date}`,
    );
  ...
  TAB.ForEachRow((row, i) => {
    ...
    if (String(row[PURCHASE_LOCATION_INDEX]).includes(PURCHASE_HEADER)) {
      const HEADER_DATE = __Util_GetDateFromDateHeader(
        String(row[PURCHASE_LOCATION_INDEX]),
      );
...
function __Util_DateInCurrentPayPeriod(compare_date: string | Date) {
  ...
  if (COMPARE_DATE.toString() === "Invalid Date") {
    return false;
  }
...
function __Util_IsHeader(str: string) {
...
function __Util_GetRowThatStartsTheMonth(override = false) {
  const CACHED_ROW_NUM = PropertiesService.getDocumentProperties().getProperty(
    START_OF_MONTH_ROW_CACHE_KEY,
  );
...
function __Util_GetRowThatEndsTheMonth(override = false) {
  const CACHED_ROW_NUM = PropertiesService.getDocumentProperties().getProperty(END_OF_MONTH_ROW_CACHE_KEY)
  if (CACHED_ROW_NUM != null && !override) {
    return Number(CACHED_ROW_NUM)
  }
  ...
  SHEET.ForEachRow((row, i) => {
    ...
    if (IsDate(PURCHASE_LOC_DATA)) {
      if (!PURCHASE_LOC_DATA.startsWith(PURCHASE_HEADER)) {
        row[PURCHASE_LOCATION_INDEX] =
          `${PURCHASE_HEADER} ${PURCHASE_LOC_DATA}`;
      }
